...
class Likelihood(object):
	...
	def __init__(self, useGPU = False):
		...
		if(self.useGPU == True):
			...
			self.GPUGetBaselineGrads = mod.get_function("getBaselineNoiseGrads")
	...
	def calculateGHSHessian(self, diagonalGHS = False):
		...
		for i in range(self.NToAs):
			...
			if(self.fitBaselineNoiseAmpPrior == True):
				...
				x0[index] = -1
				cov_diag[index] = np.sum(T1 + T2 + T3 + T4)
			...
			#Now do EVD on the dense part of the matrix
                        ...
                        if(self.BaselineNoiseParams > 0):
                                for i in range(self.NToAs):
                                        ...
                                        self.BLNEigM[i] = M2
                                        if(np.min(V2) < 0):
                                                print "Negative Eigenvalues: Poorly formed BLN Hessian", i, " using diagonal approximation\n"
						self.BLNEigM[i] = np.eye(self.BaselineNoiseParams)
						cov_diag[self.BLNList[i]] = self.BLNHess[i].diagonal()
...
#def PrintEpochParams(time):
	...
	def init_gpu_arrays(self):
		...
		if(self.incBaselineNoise == True):
			...
			self.BaselineNoiseLike_GPU  = gpuarray.zeros(self.NToAs*2*self.NFBasis, np.float64)
	...
	def addPNoise(self, Fit = True, ML = None, write=True):
		...
		self.ParamDict['PNoise'] = (pstart, pstop, MLpos, wstart, wstop, AmIInLinear, Fit)
	...
	def addPAmps(self, Fit = True, ML = None, Dense = False, write=True):
		...
		self.ParamDict['PAmps'] = (pstart, pstop, MLpos, wstart, wstop, AmIInLinear, Fit)
	...
	def addPhase(self, Fit = True, ML = None, write=True):
		...
		self.ParamDict['Phase'] = (pstart, pstop, MLpos, wstart, wstop, AmIInLinear, Fit)
	...
	def addLinearTM(self, Fit = True, ML = None, write=True):
		...
		self.ParamDict['LinearTM'] = (pstart, pstop, MLpos, wstart, wstop, AmIInLinear, Fit)
	...
	def addProfile(self, Fit = True, ML = None, write=True):
		...
		self.ParamDict['Profile'] = (pstart, pstop, MLpos, wstart, wstop, AmIInLinear, Fit)
	...
	def addScatter(self, FitScatter = True, FitFreqScale = False, MLScatter = None, MLFreqScale = None, mode='parfile', writeScatter = True, writeFreqScale = True, RefFreq = 1, Prior = 0, StepSize = 0):
		...
		self.ParamDict['Scattering'] = (pstart, pstop, MLpos, wstart, wstop, AmIInLinear, FitScatter)
		...
		self.ParamDict['ScatterFreqScale'] = (pstart, pstop, MLpos, wstart, wstop, AmIInLinear, FitFreqScale)
	...
	def addEQUAD(self, FitSignal = True, FitPrior = True, MLSignal = None, MLPrior = None, mode='flag', flag = 'sys', model = None, Dense = None, 
		...
		self.ParamDict['EQUADSignal'] = (pstart, pstop, MLpos, wstart, wstop, AmIInLinear, FitSignal)
		...
		self.ParamDict['EQUADPrior'] = (pstart, pstop, MLpos, wstart, wstop, AmIInLinear, FitPrior)
	...
	def addECORR(self, FitSignal = True, FitPrior = True, MLSignal = None, MLPrior = None, mode='flag', flag = 'sys', model = None, Dense = None, 
		...
		self.ParamDict['ECORRSignal'] = (pstart, pstop, MLpos, wstart, wstop, AmIInLinear, FitSignal)
		...
		self.ParamDict['ECORRPrior'] = (pstart, pstop, MLpos, wstart, wstop, AmIInLinear, FitPrior)
	...
	def addBaselineNoise(self, FitAmpPrior = True, FitSpecPrior = True, MLAmpPrior = None, MLSpecPrior = None, writeAmpPrior=True, writeSpecPrior=True, BaselineNoiseRefFreq = 2):
		...
		self.ParamDict['BaselineNoiseAmpPrior'] = (pstart, pstop, MLpos, wstart, wstop, AmIInLinear, FitAmpPrior)
		...
		self.ParamDict['BaselineNoiseSpecPrior'] = (pstart, pstop, MLpos, wstart, wstop, AmIInLinear, FitSpecPrior)
