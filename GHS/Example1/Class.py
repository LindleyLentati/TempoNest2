	...
	def calculateGHSHessian(self, diagonalGHS = False):
		...
		for i in range(self.NToAs):
			...
			if(self.NScatterEpochs > 0):
				...
				w = 2.0*np.pi*f
				ISS = 1.0/(self.psr.ssbfreqs()[i]**ScatterFreqScale/10.0**(9.0*ScatterFreqScale))
...
#def PrintEpochParams(time):
	...
	def GPULike(self, ndim, params):
		...
		if(self.fitScatterFreqScale == True or self.fitScatter == True):
			...
			if(self.fitScatterFreqScale == True):
				index=self.ParamDict['ScatterFreqScale'][0]
				grad[index] = np.sum(-1*SGrads*np.log(self.SSBFreqs/10**9)/np.log(10))
	...
	def addScatter(self, FitScatter = True, FitFreqScale = False, MLScatter = None, MLFreqScale = None, mode='parfile', writeScatter = True, writeFreqScale = True):
	...
	def SimArchives(self, ML, addNoise = False, outDir="./SimProfs", calcAmps=False, calcNoise=False, ASCII = False, TimeDomain = False, Norm = False):
