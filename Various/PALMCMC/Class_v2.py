...
class Likelihood(object):
    ...
    def update_progress(self, progress):
        ...
        barLength = 10 # Modify to change length of progress bar
    ...
    def loadPulsar(self, parfile, timfile, ToPickle = False,
                   FromPickle = False, root='Example'):
        ...
        for ii in range(self.numTime):
            self.TempoPriors[ii][0] = self.psr[self.psr.pars()[ii]].val
            self.TempoPriors[ii][1] = self.psr[self.psr.pars()[ii]].err / np.sqrt(redChisq)
            print "fitting for: ", self.psr.pars()[ii], \
              self.TempoPriors[ii][0], self.TempoPriors[ii][1]
        ...
        if not FromPickle:
            ...
            while profcount < self.NToAs:
                ...
                for ii in range(nsub):
                    ...
                    #print "Info:", profcount, i, nbins, nchans, \
                    #  npols, foldingperiod, inttime, centerfreq
                    ...
                    if abs(PeriodDiff) < 2.0:
                        for jj in range(nfreq):
                            ...
                            chanfreq = subint.get_centre_frequency(jj)
                            toafreq = self.psr.freqs[profcount]
                            prof=subint.get_Profile(0,jj)
                            ...
                            if np.sum(profamps) != 0 and abs(toafreq-chanfreq) < 0.001:
                                ...
                                self.ProfileInfo.append([self.SatSecs[profcount], self.SatDays[profcount],
                                                         np.float128(intsec)+np.float128(fracsecs),
                                                         pulsesamplerate, nbins, foldingperiod, noiselevel])                    
        ...
        self.ModelBats = self.psr.satSec() + self.BatCorrs -\
           self.residuals/self.SECDAY
        ...
        if self.numTime > 0:
            ...
            for ii in range(self.numTime):
                self.designMatrix[:,ii] *= self.TempoPriors[ii][1]
                zval = self.designMatrix[0,ii]
                self.designMatrix[:,ii] -= zval

            self.designMatrix = np.float64(self.designMatrix)
            N = np.diag( 1.0 / (self.psr.toaerrs * 1e-6) )
        ...
        self.ProfileStartBats = self.ProfileInfo[:,2]/self.SECDAY + \
          self.ProfileInfo[:,3]*0 + self.ProfileInfo[:,3]*0.5 + self.BatCorrs
        self.ProfileEndBats =  self.ProfileInfo[:,2]/self.SECDAY + \
          self.ProfileInfo[:,3]*(self.ProfileInfo[:,4]-1) + self.ProfileInfo[:,3]*0.5 + self.BatCorrs

        self.Nbins = (self.ProfileInfo[:,4]).astype(int)
        ProfileBinTimes = []
        for ii in range(self.NToAs):
            ProfileBinTimes.append( (np.linspace(self.ProfileStartBats[ii], self.ProfileEndBats[ii],
                                                 self.Nbins[ii])- self.ModelBats[ii]) * self.SECDAY )
    ...
    def GetProfNoise(self, profamps):
        """
        Determine an estimate of the white noise in the profile data.
        ...
        for ii in range(Nbins-Step):
            noise = np.std(profamps[ii:ii+Step])
...
def TScrunch(self, doplot=True, channels=1, FromPickle = False, ToPickle = False):
        ...
        for ii in range(channels):
            chanindices.append(zipped[(np.abs(weightsum -\
                                               np.float64(ii+1)/channels)).argmin()][0])
        ...
        averageFreqs=[]
        for ii in range(len(chanindices)-1):

            sub = zipped[np.logical_and(zipped[:,0] < chanindices[ii+1],
                                        zipped[:,0] >= chanindices[ii])]
        ...
        if not FromPickle:
                    ...
                    if abs(PeriodDiff) < 2.0:
                        for jj in range(nfreq):
                            ...
                            if np.sum(profamps) != 0 and abs(toafreq-chanfreq) < 0.001:
                                ...
                                try:
                                    value, position = min((b,a) for a,b in
                                                          enumerate(chanindices - toafreq) if b>0)
                                except:
            ...
            for ii in range(channels):
                TScrunched[ii] /= np.max(TScrunched[ii])

            self.TScrunched = TScrunched
            self.TScrunchedNoise  = np.zeros(channels)
            for ii in range(channels):
                self.TScrunchedNoise[ii] = self.GetProfNoise(TScrunched[ii])
        ...
        if doplot:
            for ii in range(channels):
                plt.plot(np.linspace(0,1,len(self.TScrunched[ii])),
                         self.TScrunched[ii])
                plt.xlabel('Phase')
    ...
    def FitEvoCoeffs(self, RFreq = 1400, polyorder = 1, doplot = False):
        ...
        for ii in range(1,Ncoeff):

            #coeffs[:][2][n::Ncoeff][refloc] = 10.0**-10
            line = np.zeros(len(coeffs[ii]))
            c = np.polynomial.polynomial.polyfit((averageFreqs-RFreq)/1000, coeffs[ii],
                                                 polyorder, rcond = None, full = False,
                                                 w = 1.0/self.TScrunchShapeErr[ii])
            NewMLShapeCoeff[ii] = c
            #print i, c, coeffs[i]

            if self.doplot == 2:
                for n in range(len(c)):
                    line += c[n] * ((averageFreqs-RFreq)/1000)**n
                    plt.errorbar((averageFreqs-RFreq)/1000, coeffs[ii],
                                 yerr = self.TScrunchShapeErr[ii], linestyle='')
    ...
    def getInitialParams(self, MaxCoeff = 1, fitNComps = 1, RFreq = 1400,
                         polyorder = 0, parameters = None, pmin = None,
                         pmax = None, x0 = None, cov_diag = None, burnin = 1000,
                         outDir = './Initchains/', sampler = 'pal', resume=False,
                         incScattering = False, mn_live = 500, doplot=False):
        ...
        if parameters is None:
            parameters=[]
            for ii in range(self.fitNComps):
                parameters.append('Phase_'+str(ii))
            for ii in range(self.fitNComps):
                parameters.append('Log10_Width_'+str(ii))
            for ii in range(self.fitNComps):
                parameters.append('NCoeff_'+str(ii))
        ...
        if sampler == 'pal':
            sampler = ptmcmc.PTSampler(ndim = n_params, logl = self.InitialLogLike,
                                       logp = self.my_prior, cov = np.diag(cov_diag**2),
            ...
            pymultinest.run(self.MNFFTInitialLogLikeWrap, self.MNprior, n_params,
                            importance_nested_sampling = False, resume = resume,
                            verbose = True, sampling_efficiency = 'model',
                            multimodal = False, n_live_points = mn_live,
        ...
        MCoeffs=[]
        for ii in range(self.fitNComps):
            CompSeps.append(ML[ii])
            Betas.append(10.0**ML[ii + self.fitNComps])
            MCoeffs.append(np.floor(10.0**ML[ii + 2*self.fitNComps]).astype(np.int))
    ...
    def Bconst(self, width, ii):
        return (1.0 / np.sqrt(width)) / np.sqrt(2.0**ii * np.sqrt(np.pi))

    def MNprior(self, cube, ndim, nparams):
        for ii in range(ndim):
            cube[ii] = (self.pmax[ii] - self.pmin[ii])*cube[ii] + self.pmin[ii]
    ...
    def FFTInitialLogLike(self, x):
        ...
        for comp in range(NComps):
            ...
            if FitNCoeff[comp] > 1:
                self.TNothpl(FitNCoeff[comp], 2*np.pi*rfftfreqs*width[comp],
                             FullMatrix[ccount:ccount+FitNCoeff[comp]])

            ExVec = np.exp( -0.5*(2*np.pi * rfftfreqs * width[comp])**2 )
            FullMatrix[ccount:ccount+FitNCoeff[comp]] = \
              FullMatrix[ccount:ccount+FitNCoeff[comp]] * ExVec * width[comp]
            ...
            for ii in range(FitNCoeff[comp]):
                FullCompMatrix[ii+ccount] *= ScaleFactors[ii]
            ...
            if self.NScatterEpochs == 0:
                ...
                MLCoeff=[]
                MLErrs = []
                for ii in range(self.TScrunchChans):

                    FFTScrunched = np.fft.rfft(self.TScrunched[ii])
                    ...
                    noise  = self.TScrunchedNoise[ii] * np.sqrt(ScrunchBins) / np.sqrt(2)

                    loglike  += -0.5*np.sum(r**2)/noise**2  - 0.5*np.log(noise**2)*len(RealFFTScrunched)

                    if self.doplot == 1:

                        bd = np.fft.rfft(self.TScrunched[ii])
                        bd[0] = 0 + 0j
                        bdt = np.fft.irfft(bd)

                        bm = np.zeros(len(np.fft.rfft(self.TScrunched[ii]))) + 0j
                    ...
                    if self.returnVal == 1:
                        ...
                        MLErrs.append(ShapeErrs*self.TScrunchedNoise[ii]/zml)
            ...
            if self.NScatterEpochs == 1:
                ...
                for ii in range(self.TScrunchChans):
                    ...
                    ScatterScale = (self.TScrunchedFreqs[ii] * 1e6)**4 / 10.0**(9.0*4.0)
                    STime = STau / ScatterScale
                    ScatterVec = self.ConvolveExp(np.linspace(0, ScrunchBins/2, ScrunchBins/2+1) /\
                                                   self.ReferencePeriod, STime)
                    ...
                    #       print ii, bin, self.TScrunched[ii][bin], s[bin], self.TScrunchedNoise[ii]
	...
	def InitialLogLike(self, x):
		...
		for ii in range(1, NComps):
			phase[ii] = (1-phase[ii-1])*phase[ii]+phase[ii-1]
		pcount += NComps
		...
		for comp in range(NComps):
			...
			if FitNCoeff[comp] > 1:
				self.TNothpl(FitNCoeff[comp], xVec,
                             FullMatrix[ccount:ccount+FitNCoeff[comp]])

			FullMatrix[ccount:ccount+FitNCoeff[comp]] *= ExVec
			for ii in range(FitNCoeff[comp]):
				FullMatrix[ii+ccount] *= ScaleFactors[ii]

			if self.doplot == 1:
				for ii in range(FitNCoeff[comp]):
					FullMatrix[ii+ccount] /= np.std(FullMatrix[ii+ccount])
		...
		if self.NScatterEpochs == 0:
			...
			for ii in range(self.TScrunchChans):
				...
				Md = np.dot(FullMatrix, self.TScrunched[ii])			
				ML = sp.linalg.cho_solve(Chol_MTM, Md)

				s = np.dot(FullMatrix.T, ML)
				r = self.TScrunched[ii] - s	

				loglike  += -0.5*np.sum(r**2)/self.TScrunchedNoise[ii]**2 -\
                   0.5*np.log(self.TScrunchedNoise[ii]**2)*ScrunchBins

                if self.doplot == 1:

				    plt.plot(np.linspace(0,1,ScrunchBins), self.TScrunched[ii])
				    plt.plot(np.linspace(0,1,ScrunchBins), s)
				    plt.xlabel('Phase')
				    plt.ylabel('Profile Amplitude')
				    plt.show()
				    plt.plot(np.linspace(0,1,ScrunchBins),self.TScrunched[ii]-s)
				    plt.xlabel('Phase')
				    plt.ylabel('Profile Residuals')
				    plt.show()
		...
		if self.NScatterEpochs == 1:
			...
			for ii in range(self.TScrunchChans):
				...
				Md = np.dot(ScatterMatrix, self.TScrunched[ii])			
				ML = sp.linalg.cho_solve(Chol_MTM, Md)

				s = np.dot(ScatterMatrix.T, ML)
				r = self.TScrunched[ii] - s	

				chanlike  = -0.5*np.sum(r**2)/self.TScrunchedNoise[ii]**2 -\
                   0.5*np.log(self.TScrunchedNoise[ii]**2)*ScrunchBins
	...
	def PreComputeFFTShapelets(self, interpTime = 1, MeanBeta = 0.1,
                               ToPickle = False, FromPickle = False, doplot = False):
		...
		numtointerpolate = np.int(self.ReferencePeriod / InterpBins / interpTime / 1e-9) + 1
		...
		if not FromPickle:
			...
			for tt in range(numtointerpolate):

				self.update_progress(np.float64(tt) / numtointerpolate)

				binpos = tt*interpStep
				...
				for comp in range(self.fitNComps):
					...
					for ii in range(self.MaxCoeff[comp]+1):
						OneCompMatrix[ii] *= ScaleFactors[ii]
					...
					for ii in range(1,self.MaxCoeff[comp]):
						OneJitterMatrix[ii] = (1.0/np.sqrt(2.0))*(np.sqrt(1.0*ii)*OneCompMatrix[ii-1] -\
                                                                  np.sqrt(1.0*(ii+1))*OneCompMatrix[ii+1]) /\
                                                                   (MeanBeta[comp]*self.ReferencePeriod)

					OneCompMatrix = OneCompMatrix.T
					OneJitterMatrix = OneJitterMatrix.T

					InterpFShapeMatrix[t][:,ccount:ccount+self.MaxCoeff[comp]]  = \
                      np.copy(OneCompMatrix[:,:self.MaxCoeff[comp]])
					InterpFJitterMatrix[t][:,ccount:ccount+self.MaxCoeff[comp]] = \
                      np.copy(OneJitterMatrix[:,:self.MaxCoeff[comp]])
	...
	def calculateShapePhaseCov(self,x):
		...
		xS = ( xS + self.ReferencePeriod/2) % (self.ReferencePeriod ) -\
           self.ReferencePeriod/2

		InterpBins = (xS%(self.ReferencePeriod / self.Nbins[:]) /\
                       self.InterpolatedTime).astype(int)
		WBTs = xS-self.InterpolatedTime*InterpBins
		RollBins = (np.round(WBTs / (self.ReferencePeriod /\
                                     self.Nbins[:]))).astype(np.int)


		# Multiply and shift out the shapelet model

		s = [np.roll(np.dot(self.InterpBasis[InterpBins[ii]][:,:NCoeff+1],
                            np.sum(((self.psr.freqs[ii] - self.EvoRefFreq)/1000.0)**np.arange(0,self.EvoNPoly+1)*ShapeAmps, axis=1)), -RollBins[ii])
                            for ii in range(len(RollBins))]

		j = [np.roll(np.dot(self.InterpJitterMatrix[InterpBins[ii]][:,:NCoeff+1],
                            np.sum(((self.psr.freqs[ii] - self.EvoRefFreq)/1000.0)**np.arange(0,self.EvoNPoly+1)*ShapeAmps, axis=1)), -RollBins[ii])
                            for ii in range(len(RollBins))]
		...
		for ii in range(self.NToAs):
			...
			smean = np.sum(s[ii]) / self.Nbins[ii] 
			s[ii] = s[ii] - smean
			j[ii] = j[ii] - smean

			sstd = np.dot(s[ii],s[ii]) / self.Nbins[ii]
			s[ii] = s[ii] / np.sqrt(sstd)
			j[ii] = j[ii] / np.sqrt(sstd)

			M = np.ones([2,self.Nbins[ii]])
			M[1] = s[ii]

			pnoise = self.ProfileInfo[ii][6]
			...
			dNM = np.dot(self.ProfileData[ii], M.T) / (pnoise*pnoise)
			...
			HessMatrix = np.zeros([HessSize, self.Nbins[ii]])

			# Phase First
			PhaseScale = MLAmp/MLSigma

			pcount = 0
			HessMatrix[pcount,:] = PhaseScale*j[ii]*self.ReferencePeriod
			pcount += 1

			fvals = ((self.psr.freqs[ii] - self.EvoRefFreq)/1000.0)**np.arange(0,self.EvoNPoly+1)

			for c in range(1, self.MaxCoeff):
				HessMatrix[pcount,:] = fvals[0]*np.roll(self.InterpBasis[InterpBins[ii]][:,c], -RollBins[ii])*MLAmp / MLSigma
				pcount += 1
    ...
    def ConvolveExp(self, f, tau, returngrad=False):
        ...
        if not returngrad:
            ...
            grad = (1.0 - tau**2*w**2)/(tau**2*w**2 + 1)**2 -\
               1j*tau*w/(tau**2*w**2 + 1)**2
    ...
    def GetScatteringParams(self, mode='parfile', flag = None, timestep = None):
		...
		if mode == 'parfile':
			...
			for ii in range(self.NScatterEpochs):
				mintime = self.psr['SXR1_'+SXList[ii][-4:]].val
				maxtime = self.psr['SXR2_'+SXList[ii][-4:]].val

				select_indices = np.where(np.logical_and( self.psr.stoas < maxtime,
                                                          self.psr.stoas >= mintime))[0]

				for index in select_indices:
					#print index, self.psr.stoas[index], SXList[i]
					if SXParamList[index] is None:
						SXParamList[index] = [ii]
					else:
						SXParamList[index].append(ii)
		...
		if mode == 'flag':
            ...
            for ii in range(self.NScatterEpochs):
				select_indices = np.where(self.psr.flagvals(flag) ==\
                                            scatterflags[ii])[0] 
				for index in select_indices:
					print index, self.psr.stoas[index], scatterflags[ii]
		...
		if mode == 'time':
            ...
            for ii in range(self.NScatterEpochs):
				select_indices = np.where(np.logical_and( self.psr.stoas < startMJD+(ii+1)*time,
                                                          self.psr.stoas >= startMJD+ii*time))[0]
				for index in select_indices:
    ...
    def PrintEpochParams(self, time=30, string ='DMX'):
        ...
        Epochs = mintime - time*0.01 + np.arange(NEpochs+1)*time
        #np.linspace(mintime-time*0.01, maxtime+time*0.01, int(NEpochs+3))
        EpochList = []

        for ii in range(len(Epochs)-1):
            select_indices = np.where(np.logical_and( stoas < Epochs[ii+1],
                                                      stoas >= Epochs[ii]))[0]
            if len(select_indices) > 0:
                print "There are ", len(select_indices), " profiles in Epoch ", ii
        ...
        for ii in range(len(EpochList)):
            if ii < 9:
                print string+"_000"+str(ii+1)+" 0 1"
                print string+"R1_000"+str(ii+1)+" "+str(Epochs[EpochList[ii]])
                print string+"R2_000"+str(ii+1)+" "+str(Epochs[EpochList[ii]+1])
                print string+"ER_000"+str(ii+1)+" 0.20435656\n"

            if ii < 99 and ii >= 9:
                print string+"_00"+str(ii+1)+" 0 1"
                print string+"R1_00"+str(ii+1)+" "+str(Epochs[EpochList[ii]])
                print string+"R2_00"+str(ii+1)+" "+str(Epochs[EpochList[ii]+1])
                print string+"ER_00"+str(ii+1)+" 0.20435656\n"

            if ii < 999 and ii >= 99:
                print string+"_0"+str(ii+1)+" 0 1"
                print string+"R1_0"+str(ii+1)+" "+str(Epochs[EpochList[ii]])
                print string+"R2_0"+str(ii+1)+" "+str(Epochs[EpochList[ii]+1])
                print string+"ER_0"+str(ii+1)+" 0.20435656\n"

            if ii < 9999 and ii >= 999:
                print string+"_"+str(ii+1)+" 0 1"
                print string+"R1_"+str(ii+1)+" "+str(Epochs[EpochList[ii]])
                print string+"R2_"+str(ii+1)+" "+str(Epochs[EpochList[ii]+1])
                print string+"ER_"+str(ii+1)+" 0.20435656\n"
    ...
    def WaterFallPlot(self, ML):
        ...
        im1 = ax1.imshow(np.log10(np.abs(Data)), interpolation="none",
                         extent=(np.min(x), np.max(x), 0, 1), aspect=aspect)
        ...
        im2 = ax2.imshow(np.abs(Res), interpolation="none",
                         extent=(np.min(x), np.max(x), 0, 1), aspect=aspect)
